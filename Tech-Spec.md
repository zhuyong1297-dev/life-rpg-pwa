...
- JSON schema 4 备份包含 `appVersion`、`exportedAt` 和六张表的完整内容，并兼容读取 schema 1～3 以及 V2.3.0 的 schema 4 备份。
- Zod 先在事务外校验结构和业务约束。
...
- 今天页手机端为单栏，桌面端为约 720px 任务主栏与 260 至 300px 状态右栏。
...
- Playwright：完成证据、反馈速度与收缩态、四页导航、创建和编辑弹窗、三项关键行动满载、响应式、备份恢复、离线启动和 PWA 资源。
