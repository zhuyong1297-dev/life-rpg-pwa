...
- JSON schema 5 备份包含 `appVersion`、`exportedAt` 和六张表的完整内容，并兼容读取 schema 1～4；旧备份恢复时以备份账本总 XP 初始化等级基线，缺少反馈强度时补为 `clear`。
- 导入时校验当前奖励目标必须指向启用商品。
- Zod 先在事务外校验结构和业务约束。
...
- 创建表单通过原生 `details` 渐进展示属性和难度；三层高级目标继续使用现有分段控件。
...
- 角色页只常驻 88px 等级摘要、最近 7 天最多 3 条成长记录和约 104px 商店摘要；完整路线、按月旅程档案和商店目录使用移动端底部弹层、桌面居中弹窗。
- 月度旅程章节从追加式流水派生，默认加载 3 个月、每次追加 6 个月；同一 completion 的层次奖励合并为一次完成，不新增汇总表。
...
- Vitest：奖励、等级路线、成长报告、月度旅程、三档 Web Audio 与振动、目标、关键行为上限、余额、撤销和幂等。
- fake-indexeddb：事务原子性、商品管理与目标唯一性、里程碑固化与礼券领取、双击防重、旧快照升级、当天取消和 schema 1～5 导入回滚。
- Playwright：完成证据、2/3/4 音语义、三档反馈、紧凑路线、旅程档案、商品管理、五栏移动导航、响应式、离线启动和 PWA 资源。
