...
test('三层时间习惯一键选层并在当天只补 XP 差额', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '创建', exact: true }).click()
  await expect(page.getByText(/基础 5 分钟 · 标准 20 分钟 · 突破 45 分钟/)).toBeVisible()
  ...
  await page.getByRole('button', { name: '升级 示例分层习惯' }).click({ force: true })
  ...
  await expect(page.locator('.character-progress > strong')).toHaveText('10 XP')
  await expect(page.locator('.coin-balance strong')).toHaveText('5')
})

test('活动管理可以把单次习惯转换为三层次数目标', async ({ page }) => {
  await page.getByRole('button', { name: '创建行动' }).click()
  await page.getByLabel('名称').fill('示例次数习惯')
  await page.getByRole('button', { name: '创建', exact: true }).click()
  await page.getByRole('button', { name: '设置' }).click()
  await page.getByTitle('设置习惯目标').click()
  await page.getByRole('button', { name: '三层目标' }).click()
  ...
  await expect(page.getByText('习惯目标已更新，历史记录保持不变')).toBeVisible()
