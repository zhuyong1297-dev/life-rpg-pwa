...
- JSON schema 5 备份包含 `appVersion`、`exportedAt` 和六张表的完整内容，并兼容读取 schema 1～4；旧备份恢复时以备份账本总 XP 初始化等级基线。
- Zod 先在事务外校验结构和业务约束。
...
- 完成音效由单例 Web Audio `AudioContext` 合成；普通完成使用双音，角色升级使用三音，页面从后台恢复时调用 `resume()`。
...
- Vitest：奖励、等级路线、成长报告、Web Audio 状态、目标、关键行为上限、余额、撤销和幂等。
- fake-indexeddb：事务原子性、里程碑固化与礼券领取、双击防重、旧快照升级、当天取消和 schema 1～5 导入回滚。
- Playwright：完成证据、双音与升级三音、反馈速度与收缩态、五栏移动导航、奖励路线、创建和编辑弹窗、响应式、离线启动和 PWA 资源。
