...
- 周复盘为分层目标保存二元或三元层次分布，并可保存最低次数、最低时间秒数和次数单位；新条目同时保存可选标题与属性快照。
...
2. 在同一 Dexie 事务内为旧 completion 补齐名称、属性、难度和版本快照，为旧 review item 补齐标题和属性快照。
...
- JSON schema 5 备份包含 `appVersion`、`exportedAt` 和六张表的完整内容，接受 V2.5.0、V2.6.0 与 V2.7.0 并兼容读取 schema 1～4；恢复旧备份时保留当前设备的游戏日启用状态。
- 导入时校验当前奖励目标必须指向启用商品。
- Zod 先在事务外校验结构和业务约束。
...
- 关键行动渲染为委托任务条；普通习惯和一次性任务复用三行组件，完整展示标题/难度/层次、属性/频率、目标/奖励并为操作按钮保留固定轨道。
...
- fake-indexeddb：事务原子性、游戏日过渡、商品管理、里程碑固化、双击防重、旧快照升级、永久移除定义、当前游戏日取消和 schema 1～5 导入回滚。
- Playwright：两/三层选级、完成证据、2/3/4 音语义、活动管理分组与搜索、三行日常委托、月历行动日志、商品管理、五栏移动导航、响应式、离线启动和 PWA 资源。
