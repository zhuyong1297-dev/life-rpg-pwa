...
test('每周次数目标可以连续记录、跨层发奖并撤销最近一次', async ({ page }) => {
  ...
  const record = page.locator('.activity-row').filter({ hasText: '每周写作' }).locator('.incremental-record-button')
  await record.click()
  ...
  await expect(page.locator('.activity-row').filter({ hasText: '每周写作' }).locator('.weekly-progress-summary')).toContainText(/距离突破层：3\/5次/)

  await page.getByRole('button', { name: /查看本周记录与撤销/ }).click({ force: true })
...
test('组合累计目标用默认时长一键记录并可选择备用时长', async ({ page }) => {
  await page.getByRole('button', { name: '创建行动' }).click()
  ...
  const row = page.locator('.activity-row').filter({ hasText: '每周训练' })
  await row.locator('.incremental-record-button').click()
  await expect(page.getByText('+3 XP', { exact: true })).toBeVisible()
  await row.getByRole('button', { name: '选择其他记录时长' }).click({ force: true })
  await row.getByRole('button', { name: '10秒' }).click()
  await expect(row.locator('.weekly-progress-summary')).toContainText(/标准层：1\/3 次达到每次 20秒/)
})
