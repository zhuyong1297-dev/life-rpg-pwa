...
- JSON schema 6 备份包含 `appVersion`、`exportedAt` 和七张表的完整内容，并兼容读取 schema 1～5；恢复旧备份时补为空赛季列表并保留当前设备的游戏日启用状态。
- 导入时校验当前奖励目标必须指向启用商品。
- Zod 先在事务外校验结构和业务约束。
...
- fake-indexeddb：事务原子性、游戏日过渡、赛季唯一性、建议响应不改活动、商品管理、里程碑固化、永久移除定义和 schema 1～6 导入回滚。
