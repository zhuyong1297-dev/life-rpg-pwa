...
async function chooseTraveler(page: import('@playwright/test').Page, name: '男性旅者' | '女性旅者' = '男性旅者') {
  const dialog = page.getByRole('dialog', { name: '选择你的旅者' })
...
test('全新空库选择领域、预览推荐并完成首次行动', async ({ page }) => {
  ...
  await expect(travelerDialog.getByRole('button', { name: /男性旅者/ })).toBeVisible()
  await expect(travelerDialog.getByRole('button', { name: /女性旅者/ })).toBeVisible()
  await travelerDialog.getByRole('button', { name: /女性旅者/ }).click()
