...
test('本周里程碑与时长选择在各视口保持分层且不重叠', async ({ page }, testInfo) => {
  await openApp(page)
  await page.getByRole('button', { name: '创建行动' }).click()
  await page.getByLabel('名称').fill('每周耐力与恢复训练计划')
  ...
  const row = page.locator('.weekly-progress-item').filter({ hasText: '每周耐力与恢复训练计划' })
  await expect(page.locator('.weekly-key-card').filter({ hasText: '每周耐力与恢复训练计划' })).toBeVisible()
  await expect(row.locator('.weekly-milestone-node')).toHaveCount(3)
  const layout = await row.evaluate((element) => {
    const track = element.querySelector<HTMLElement>('.weekly-milestone-track')!.getBoundingClientRect()
    const actions = element.querySelector<HTMLElement>('.weekly-progress-actions')!.getBoundingClientRect()
    return { trackBottom: track.bottom, actionsTop: actions.top, rowWidth: element.getBoundingClientRect().width, scrollWidth: element.scrollWidth }
  })
  expect(layout.actionsTop).toBeGreaterThanOrEqual(layout.trackBottom)
  expect(layout.scrollWidth).toBeLessThanOrEqual(layout.rowWidth + 1)
  await expectNoHorizontalOverflow(page)
  await page.screenshot({ path: `test-results/weekly-progress-${testInfo.project.name}.png`, fullPage: true })

  await row.getByRole('button', { name: '选择时长' }).click()
  const picker = page.getByRole('dialog', { name: '每周耐力与恢复训练计划' })
  await expect(picker.getByRole('button', { name: /30秒.*常用/ })).toBeVisible()
  await expect(picker.getByRole('button', { name: '10秒', exact: true })).toBeVisible()
  await expectNoHorizontalOverflow(page)
  await page.screenshot({ path: `test-results/duration-picker-${testInfo.project.name}.png` })
})
