...
test('每日评分习惯选择分数即完成，改分不会重复发奖', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '记录 1–5 分 每日恢复体验' }).click()
...
test('待启动的试跑评分可从今天页启用，晨起时直接选择 1–5 分', async ({ page }) => {
  await page.evaluate(async () => {
    const current = new Date()
    current.setHours(current.getHours() - 4)
    const today = [
    ...
    const transaction = database.transaction(['activities', 'settings'], 'readwrite')
    const oldActivity = {
      id: 'e2e-old-morning-tier',
      title: '晨起评分验证',
      scheduledTime: '07:30',
      cue: '起床后、查看信息前',
      protocol: '记录醒来后的真实恢复感。',
      type: 'habit',
      domain: 'health',
      difficulty: '简单',
      goal: { kind: 'tiered', metric: 'count', unit: '项', thresholds: [1, 2] },
      schedule: { kind: 'daily' },
      isKey: true,
      enabled: true,
      revision: 1,
    ...
    const trialId = 'trial:e2e-rating-restart'
    transaction.objectStore('activities').put(oldActivity)
    transaction.objectStore('settings').put({
      key: 'applicationTrial',
      value: {
        id: trialId,
        version: 1,
        applicationId: 'app-e2e-rating-restart',
        sourcePackageId: 'e2e.rating-restart.v1',
        sourcePlanId: 'application-plan:e2e-rating-restart',
        title: '睡眠恢复实验',
        successCriterion: '连续记录真实恢复感',
        baseline: '缺少可比较的晨起体验记录',
        targetOutcome: '形成七天恢复感证据',
        outcomeIndicator: '每天晨起选择 1–5 分',
        knowledge: {
          primary: {
            kind: 'principle',
            title: '先记录真实体验',
            reference: 'Knowledge/虚构睡眠实验.md',
            principle: '先记录，再判断。',
          },
          supporting: [],
        },
        ...
        focusActivities: [{
          ...
          scheduledTime: oldActivity.scheduledTime,
          cue: oldActivity.cue,
          protocol: oldActivity.protocol,
          domain: oldActivity.domain,
          difficulty: oldActivity.difficulty,
          goal: oldActivity.goal,
    ...
    transaction.objectStore('settings').put({
      ...
      value: {
        ...
        replacements: [{
          sourceActivityId: oldActivity.id,
          title: oldActivity.title,
          scheduledTime: oldActivity.scheduledTime,
          cue: oldActivity.cue,
          protocol: oldActivity.protocol,
  ...
  const activation = page.getByRole('region', { name: '晨起评分状态' })
  await expect(activation).toContainText('评分模式尚未启用')
  await expect(activation).toContainText('夜间的赛季状态记录与这项习惯评分无关')
  await activation.getByRole('button', { name: '启用评分并重启试跑' }).click()

  const ratingButton = page.getByRole('button', { name: '记录 1–5 分 晨起评分验证' })
  await expect(ratingButton).toBeVisible()
  await ratingButton.click()
  const ratingDialog = page.getByRole('dialog', { name: '今天醒来后的恢复感如何？' })
  await expect(ratingDialog).toBeVisible()
  await ratingDialog.locator('.rating-score-grid button').nth(3).click()
  await expect(page.locator('.v5-feedback')).toContainText('今天醒来后的恢复感如何？ 4/5')
  ...
  const persisted = await page.evaluate(async () => {
    ...
    const transaction = database.transaction(['activities', 'completions', 'settings'], 'readonly')
    const getAll = <T,>(storeName: string) => new Promise<T[]>((resolve, reject) => {
      const result = transaction.objectStore(storeName).getAll()
      result.onsuccess = () => resolve(result.result as T[])
      result.onerror = () => reject(result.error)
    })
    const activities = await getAll<{ id: string; goal: { kind: string }; archivedAt?: string }>('activities')
    const completions = await getAll<{ activityId: string; ratingValue?: number }>('completions')
    const settings = await getAll<{ key: string }>('settings')
    database.close()
    const rating = activities.find((activity) => activity.goal.kind === 'rating')
    return {
      oldArchived: Boolean(activities.find((activity) => activity.id === 'e2e-old-morning-tier')?.archivedAt),
      ratingValue: completions.find((completion) => completion.activityId === rating?.id)?.ratingValue,
      restartPending: settings.some((setting) => setting.key === 'applicationTrialRestart'),
    }
  })
  expect(persisted).toEqual({ oldArchived: true, ratingValue: 4, restartPending: false })
})
